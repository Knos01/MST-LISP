...
(defun heap-insert (heap-id K V)
  (if (is-heap heap-id)
      (cond ((gethash (list heap-id V) *heap-entries*) ; se il vertice � dentro HEAP
            ...
            (t (and ; aggiungo un nuovo heap entry 
                ...
                (setf (gethash (list heap-id V) *heap-entries*) ;aggiunge entry
                      (list K (1- (heap-size heap-id)) (get-parent heap-id (1- (heap-size heap-id)))))                (heapify heap-id (heap-size heap-id) (1- (heap-size heap-id))))))))

(defun heapify (heap-id s i)
  (cond ((= (heap-size heap-id) 0) (format t "Caso base"))
        ; se il figlio � minore di parent, swap
        ((< 
